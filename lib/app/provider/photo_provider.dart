...
Future<T> elapsedFuture<T>(
  ...
  stopwatch.stop();
  //debugPrint('${prefix != null ? '$prefix: ' : ''}${stopwatch.elapsed}');
  return result;
}
...
class PhotoProvider extends ChangeNotifier {
  ...
  bool get isSelectMultiple => _isSelectMultiple;

  set isSelectMultiple(bool value) {
...
class AssetPathProvider extends ChangeNotifier {
  ...
  Future<void> onRefresh() async {
    //debugPrint('is refreshing $refreshing');
    if (refreshing) {
      return;
    }
    refreshing = true;
    ...
    //debugPrint('list in photo provider ${this.list.first}');
  ...
  Future<void> delete(AssetEntity entity) async {
    try {
      ...
      //debugPrint('error in delete photo provider: $e');
    }
  }
  ...
  Future<void> deleteSelectedAssets(List<AssetEntity> entity) async {
    try {
      ...
      //debugPrint('error from delete selected assets: $e');
    }
  }
  ...
  void printListLength(String tag) {
    //debugPrint('$tag length : ${list.length}');
  }
}
