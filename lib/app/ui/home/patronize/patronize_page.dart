...
class PatronizePage extends StatefulWidget {
  const PatronizePage({Key? key}) : super(key: key);

  @override
  State<PatronizePage> createState() => _PatronizePageState();

  static void showPanel(BuildContext context, bool isShowed, String? momentId,
      {bool isDelete = false}) async {
    _PatronizePageState? state =
        context.findAncestorStateOfType<_PatronizePageState>();
    if (state != null) {
          ...
          child: BlocBuilder<ParentBloc, ParentState>(
            builder: (context, state) {
              ...
              if (state is ParentLoaded) {
                ...
                _model = state.parent;
                if (_model.patronizeds.isNotEmpty) {
                  // BlocProvider.of<MomentBloc>(context).add(
                  //     LoadPatronizeMoments(patronizeds: _model.patronizeds));
                  BlocProvider.of<MomentBloc>(context).add(
                      FirstPatronizeListMoments(
                          patronizeds: _model.patronizeds, limit: 8));
                } else {
                  _list.value.clear();
                  _grid.value.clear();
                  return _buildHallUi();
              ...
              return BlocBuilder<MomentBloc, MomentState>(
                builder: (context, state) {
                  ...
                  if (state is MomentListLoaded) {
                    ...
                    BlocProvider.of<MomentBloc>(context).add(
                        FirstPatronizeTileMoments(
                            patronizeds: _model.patronizeds, limit: 32));
                    // }
                  }
                  ...
                  //   BlocProvider.of<MomentBloc>(context).add(
                  //       LoadPatronizeMomentsByParentId(
                  //           patronizeds: _model.patronizeds,
                  ...
                  //   print('my loaded');
                  //   _myPatronize = state.moments;
                  //   _list.value.clear();
                  //   _grid.value.clear();
                  //   _list.value.addAll(_tempList);
                  //   _list.value.addAll(_myPatronize);
                  //   _grid.value.addAll(_tempList);
                  //   _grid.value.addAll(_myPatronize);
                  //   print("list value: " + _list.value.length.toString());
                  //   if (context.loaderOverlay.overlayWidgetType !=
                  //       ReconnectingWidget) {
                  //     context.loaderOverlay.hide();
                  //   }
