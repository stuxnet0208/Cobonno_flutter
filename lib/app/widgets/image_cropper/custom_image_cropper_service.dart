import 'package:cobonno/app/widgets/image_cropper/custom_crop.dart';
...
import 'package:multi_image_crop/src/common/widgets/loader_widget.dart';

class CustomMultiImageCropService extends StatefulWidget {
  const CustomMultiImageCropService(
      {Key? key,
      required this.files,
      this.activeColor,
  ...
  @override
  State<CustomMultiImageCropService> createState() => _CustomMultiImageCropServiceState(files);
}

class _CustomMultiImageCropServiceState extends State<CustomMultiImageCropService>
    with SingleTickerProviderStateMixin {
  /// [cropKeyList] contains Global key of each crop ui.
  late List<GlobalObjectKey<CustomCropState>> cropKeyList = [];
  ...
  _CustomMultiImageCropServiceState(this.files);
  ...
  void initState() {
    ...
    cropKeyList = List.generate(files.length, (index) => GlobalObjectKey<CustomCropState>(index));
  ...
  PreferredSizeWidget utilityAppBar() {
    return AppBar(
      title: Text(
        AppLocalizations.of(context).cropper,
      ...
      backgroundColor: CustomColors.primaryColor,
      leading: IconButton(
        icon: Icon(isIos ? CupertinoIcons.back : Icons.arrow_back, color: Colors.white),
        onPressed: () {
  ...
  Widget croppingView() {
    String extension =
        files.isNotEmpty ? files[currentPage].path.split('.').last.toLowerCase() : '';
    return Expanded(
        child: Stack(
      children: [
  ...
  cropImage() async {
    showLoaderDialog(context, title: AppLocalizations.of(context).loading);
